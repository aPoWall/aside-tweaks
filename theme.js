...
// бумажная база инструментов: тёплый лист на свету, тёплые чернила в темноте
const PAPER = {
  light: { base: '#f4f1e7', soft: '#faf8f2', line: '#ddd8c9', fg: '#1b1a17', muted: '#8d887a', dim: '#6b675c', sel: '#e9e5d8' },
  dark: { base: '#1a1a18', soft: '#232320', line: '#33322d', fg: '#ece9e0', muted: '#84806f', dim: '#a09b8c', sel: '#2b2a26' }
};
...
function applyTheme(themeRaw) {
  ...
  s.setProperty('--bg', mix(t.accent, p.base, t.tint));
  s.setProperty('--bg-soft', mix(t.accent, p.soft, Math.min(100, t.tint * 1.4)));
  s.setProperty('--tile', mix(t.accent, p.soft, Math.min(100, t.tint * 1.4)));
  s.setProperty('--tile-fg', p.fg);
  s.setProperty('--line', mix(t.accent, p.line, Math.min(100, t.tint * 1.8)));
  s.setProperty('--fg', p.fg);
  s.setProperty('--muted', p.muted);
  s.setProperty('--dim', p.dim);
  s.setProperty('--sel', mix(t.accent, p.sel, Math.min(100, t.tint * 2)));
}
